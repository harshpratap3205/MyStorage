...
function DirectoryView() {
  ...
  async function getdata() {
    try {
      setIsLoading(true);
      const response = await fetch(`http://localhost:4001/directory/${dirname}`);
  ...
  const uploadHandler = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4001/file/${selectedFile.name}`, {
        method: 'POST',
        body: formData
      });
  ...
  async function deleteHandler(fileName) {
    try {
      const response = await fetch(`http://localhost:4001/file/${fileName}`, { 
        method: "DELETE" 
      });
  ...
  async function renameFileHandler(oldName, newName) {
    try {
      const response = await fetch(`http://localhost:4001/file/${oldName}`, { 
        method: "PATCH",
                      ...
                      <div className="flex space-x-2">
                        {editingFile === data ? (
                          ...
                          <>
                           ...
                           {!isDirectory?<a href={`http://localhost:4001/file/${data}?action=open`} >
                              <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                           ...
                           { !isDirectory?<a href={`http://localhost:4001/file/${data}?action=download`} download>
...
//   const [uploadStatus, setUploadStatus] = useState('') // Added state for upload status

//   async function getdata() {
//     const response = await fetch("http://localhost:4001/directory")
//     const personInfo = await response.json()
//     setData(personInfo)
//   }
...
//   async function deleteHandler(fileName) {
//     const response = fetch(`http://localhost:4001/file/${fileName}`, { method: "DELETE" })
...
//   async function renameFileHandler(oldName, newName) {
//     const response = fetch(`http://localhost:4001/file/${oldName}`, { 
//       method: "PATCH",
//       headers: { "Content-type": "application/json" },
//       body: JSON.stringify({ newName })
//     })
...
//   async function saveRename(oldName, newName) {
//     renameFileHandler(oldName, newName)
//     setData(prevData => 
//       prevData.map(name => name === oldName ? newName : name)
//     )
//     setEditingFile(null)
//     setNewFileName('')
//   }
...
//             {/* Updated form with submit handler */}
//             <form onSubmit={uploadHandler} className="mt-4">
//               <label 
//                 htmlFor="file" 
//                 className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-blue-300 rounded-lg cursor-pointer bg-blue-50 hover:bg-blue-100 transition-colors"
...
//                     ) : (
//                       <>
//                         <a href={`http://localhost:4001/file/${data}?action=open`}>
//                           <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center">
//                             <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
//                             </svg>
...
//                           </button>
//                         </a>
//                         <a href={`http://localhost:4001/file/${data}?action=download`}>
//                           <button className="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center">
//                             <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
//                             </svg>
...
//                           Rename
//                         </button>
//                         <button onClick={() => { deleteHandler(data) }} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center">
//                           <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
//                           </svg>
//                           Delete
...
//   async function deleteHandler(fileName) {
//     console.log("delete ")
//     const response = fetch(`http://localhost:4001/${fileName}`, { method: "DELETE" })
//     const data = await (await response).json()
//     console.log(data)
//     getdata()
//   }
...
//   function renameHandler(fileName) {
//     console.log("Rename clicked for:", fileName)
//     setEditingFile(fileName)
//     setNewFileName(fileName)
...
//     console.log(oldName)
//     console.log(newName)
//     const response = fetch(`http://localhost:4001/${oldName}`, { method: "PATCH",
//       headers:{
...
//   }
//   async function saveRename(oldName, newName) {
//     // console.log("Saving new name:", newName, "for file:", oldName)
//     renameFileHandler(oldName,newName)
//     setData(prevData => 
//       prevData.map(name => name === oldName ? newName : name)
//     )
...
//                   <p className="text-xs text-gray-500">Select your file here</p>
//                 </div>
...
//                     ) : (
//                       <>
//                         <a href={`http://localhost:4001/${data}?action=open`}>
//                           <button className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center">
//                             <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
//                             </svg>
...
//                           </button>
//                         </a>
//                         <a href={`http://localhost:4001/${data}?action=download`}>
//                           <button className="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center">
//                             <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
//                               <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
//                             </svg>
